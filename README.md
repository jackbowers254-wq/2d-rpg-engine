...
| Banded lighting + palette swap | Autotiled lake | Parallax title |
|---|---|---|
| ![lighting](docs/shot_lighting.png) | ![autotile](docs/shot_autotile.png) | ![title](docs/shot_title.png) |
| Animated overworld | CRT post-FX | Quest log |
| ![overworld](docs/shot_anim.png) | ![postfx](docs/shot_postfx.png) | ![quest](docs/shot_questlog.png) |

*(All demo art is generated — Aseprite-format sprite sheets via
`tools/gen_demo_art.py`, audio via `tools/gen_demo_audio.py`, and everything else
from coloured boxes + palettes. An entity with no animation still falls back to a
...
### Gen 2 Phase B sections
| Key | Default | Meaning |
|---|---|---|
| `scripting.scripts_path` | `"data/scripts"` | Cutscene/script files. |
| `quests.quests_path` | `"data/quests"` | Quest definitions. |
| `economy.shops_path` | `"data/shops"` | Shop definitions. |
| `economy.starting_currency` | `20` | Gold a new game starts with. |
| `audio.crossfade_ms` | `700` | Music crossfade time on track change. |
| `combat.abilities_path` / `combat.statuses_path` | `data/abilities` / `data/statuses` | Skill + status data. |
| `combat.player_mp` | `12` | Starting/maximum MP. |
| `render.effects_enabled` | `true` | Master toggle for lighting + particles. |
| `render.torch_radius` / `torch_color` | `56` / warm | Player light in dark maps. |
| `render.particle_size` | `2` | Particle pixel size (chunky). |
| `render.light_bands` | `4` | **(C)** Quantize light falloff into N steps (0 = smooth). |
| `render.day_night` | `{enabled,period,start,keyframes}` | **(C)** Day/night ambient cycle for outdoor maps. |
| `render.post.*` | all off | **(C)** `scanlines`/`vignette`/`bloom`/`chromatic`/`color_grade` toggles + params (F2 in-game). |
| `assets.palettes_path` | `"data/palettes"` | **(C)** Palette swaps + palettes. |

Per-map (C): `ambient` (indoor dark), tile-layer property `autotile`+`edge_color`,
`parallax` data; per-sprite `palette` swap id.

Per-map (Tiled map custom properties): `music` (track), `ambient` (colour; dark =
needs lighting), `display_name`. Map objects: `type:"trigger"` (event/script/once/
requires), `type:"light"` (color/radius), `type:"spawn"` (entity + overrides),
`type:"portal"`, `type:"player_start"`.
...
## Gen 2 features (Phase B)

Phase B adds gameplay systems on top of Phase A. All are data-driven and
backward-compatible.

### Event / trigger / scripting (cutscenes in data)
Define scripted sequences as JSON action lists (`data/scripts/*.json`) fired by
**triggers** placed in maps (Tiled objects, `type = "trigger"`):

- Trigger `event`: `enter` (walk into the area), `interact` (stand on it + confirm),
  or `load` (on map entry). Optional `once`, `requires`/`unless` (flag gates).
- Actions (sequential; some block): `set_flag`, `set_var`, `give_item`/`take_item`,
  `add_currency`, `face`, `move_entity`, `teleport`, `spawn`, `remove_entity`,
  `wait`, `dialogue`, `start_battle`, `play_music`/`play_sound`, `emit`,
  `lock_player`/`unlock_player`, `start_quest`/`advance_quest`/`complete_quest`,
  `if`/`stop`. Conditions use flags/items/quests/vars with `not`/`all`/`any`.

The runner cooperates with the scene stack (a `dialogue` action pushes the box and
resumes when it closes) and locks the player during cutscenes. Add an action with
`@script_action("name")`. The demo's intro cutscene (`data/scripts/intro.json`)
fires when you first approach the village. See `engine/scripting/`.

### Quests + log UI
Quests (`data/quests/*.json`) have objectives that auto-advance from events
(`defeat` via `entity_died`) or polling (`flag`, `collect`). Completion grants
xp/currency/item rewards and fires `quest_completed`. State lives in
`GameState.quests` (saved). Open the log with **L** or the pause menu. The intro
cutscene starts "Clear the Cave".

### Audio (crossfade music, layered SFX, volumes)
`game.audio` (AudioManager) crossfades background music (map `music` property;
battle swaps and the overworld restores), layers SFX (pickup, hits), and exposes
live `master`/`music`/`sfx` volumes (pause → **Options**). Falls back to silent if
no device. Demo audio is generated by `tools/gen_demo_audio.py` (stdlib WAVs).

### Lighting + particles (via the render hooks)
Config-gated (`render.effects_enabled`). `LightingEffect` darkens by a per-map
`ambient` colour and adds point lights (a player torch + map `light` objects — see
the cave). `ParticleSystem` emits chunky bursts on hits/pickups. Disable with
`render.effects_enabled=false`.

### Smarter AI (A* + aggro)
`engine/ai` provides grid A* + line-of-sight. The `hunter` AI behaviour detects the
player within `aggro_range` (optional LOS), paths around walls toward them, and
gives up when they escape. The cave's `enemy_hunter` demonstrates it.

### Abilities + status effects
Skills (`data/abilities/*.json`: attack/heal/buff, MP cost, optional status) and
status effects (`data/statuses/*.json`: damage-over-time, stat modifiers, duration)
wired into turn-based combat. The player has Power Strike / Heal / Poison Dart /
Guard (battle → **Skill**). MP persists in the save.

### Economy (shops)
Shops (`data/shops/*.json`) buy/sell items against `GameState.currency`. Talk to the
Merchant in the overworld (interactable `action: "shop"`). Buy/Sell tabs, prices from
item values × markup / sell ratio.

---

## Gen 2 features (Phase C) — pixel-art pipeline

A professional pixel-art rendering pipeline. Everything is config-gated; a simple
game can leave it off.

### Palette system
Data-defined colour swaps (`data/palettes/*.json`) recolour shared art at runtime
(`SpriteComponent.palette`) — the demo's red hunter reuses the green slime sprite.
The `PaletteManager` also quantizes surfaces to a palette with optional ordered
dithering. `game.palettes`.

### Pixel-art lighting
The lighting pass quantizes its falloff into bands (`render.light_bands`) for crisp
light pools instead of smooth gradients, and a **day/night cycle**
(`render.day_night`, keyframed ambient) darkens outdoor maps over time so the
torch matters at night. Indoor maps (cave) keep a fixed dark `ambient`. (Normal
maps are a documented extension hook — better suited to a GPU backend.)

### Retro post-processing (off by default)
`engine/graphics/postprocess.py` applies CRT **scanlines**, **vignette**, **bloom**,
**chromatic aberration** and **colour grading** on the CPU to the base surface
(~3 ms). Each toggles in `render.post`; press **F2** for a CRT preset in-game.
Screen **transitions** (fade/wipe/pixel) ship from Phase A.

### Autotiling
A tile layer with `autotile: true` (Tiled property) auto-selects edge/corner
variants from neighbours — the demo's lake gets a shoreline with no hand-placed
edges. Art-free (variants generated from the tile colour); for real art the same
neighbour masks index a 47-tile blob set.

### Parallax backgrounds
`engine/graphics/parallax.py`: multi-layer backdrops with per-layer scroll `factor`
(data-driven `fill`/`band`/`bumps`/`dots`/`image` layers). The title screen uses
`data/parallax/title.json` (starfield + 3 hill layers).

### Polish
White **hit-flash** on damage, **screen shake** on hits (overworld + battle), and
chunky (pixel-sized) particles.

### Art preview
`python tools/preview.py [sheet.json]` — cycle clips, **Tab** palette swaps,
**Shift** a light, **F2** post-fx; inspect imported art under every system.
